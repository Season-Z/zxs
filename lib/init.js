...
const { tmpDir, cacheDir } = require('../utils/config')
...
async function remoteTmpList() {
  ...
  // 本地缓存
  const temPath = path.join(cacheDir, '/tmp-list.txt')
  await fs.writeFile(temPath, tmpList)
...
async function getTemplateList(params) {
  ...
  const exsitDir = fs.existsSync(cacheDir)
